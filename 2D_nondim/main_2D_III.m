...
% computes the MP model (4.4.39 of the thesis) in 2-D (non-dimensional form) and saves the results
% in the folder Plots_III and Videos_III
% source term for tumor  = -c1*c*n*(h -1)
% source term for normal cells as given by -(c2*m(h-1)_+ )/(1 + m+ h)  - c3*m
% in this case there are only two occlusion sites
% The corresponding reults are included in Figure 4.4 of the thesis
clear all;
clc;
close all;
tic
...
% c_old       =  0.95*(exp(-exponent)+exp(-exponent2)+exp(-exponent3));
...
exponent2 = ((X-centerx2).^2 + (Y-centery2).^2)./(2*sigma2^2);
exponent3 = ((X-centerx3).^2 + (Y-centery3).^2)./(2*sigma3^2);
% n_old       =  0.05*(exp(-exponent)+exp(-exponent2)+exp(-exponent3));
n_old       =  0.9*(exp(-exponent)+exp(-exponent2));
n_old = n_old';
...
exponent2 = ((X-centerx2).^2 + (Y-centery2).^2)./(2*sigma2^2);
exponent3 = ((X-centerx3).^2 + (Y-centery3).^2)./(2*sigma3^2);
% h_old       =  10^(-7)*exp(-exponent)+10^(-7)*exp(-exponent2)+10^(-6.4)*exp(-exponent3);
h_old       =  (1/h_max)*(10^(-7)*exp(-exponent)+10^(-7)*exp(-exponent2));
h_old = h_old';
...
tic;
%% Videos
% In this section Videos are saved in Videos_III folder
% To check if Videos_III folder exists otherwise to create
if not(isfolder('Videos_III'))
    mkdir('Videos_III')
end
...
videofile = VideoWriter(strcat('Videos_III/Tumor_III', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
for i = 1: size(c_total,3)
    figure(4)
    surf(x,y,c_total(:,:,i)')
    view(0,90)
    colorbar
...
videofile = VideoWriter(strcat('Videos_III/Acidity_III', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
for i = 1: size(h_total,3)
    figure(5)
    surf(x,y,h_total(:,:,i)')
    view(0,90)
    colorbar
...
videofile = VideoWriter(strcat('Videos_III/Normal_III', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
for i = 1: size(m_total,3)
    ...
    drawnow
    if (i==1)
        title('Initial normal tissue', 'Fontsize', 15);
    else
        title(['Normal tissue at t = ', num2str(3*(i-1)),' days '], 'Fontsize', 15);
    end
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
...
videofile = VideoWriter(strcat('Videos_III/Necrotic_III', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
for i = 1: size(n_total,3)
    figure(8)
    surf(x,y,n_total(:,:,i)')
    view(0,90)
    colorbar
    ...
    drawnow
    if (i==1)
        title('Initial necrotic matter', 'Fontsize', 15);
    else
        title(['Necrotic matter at t = ', num2str(3*(i-1)),' days '], 'Fontsize', 15);
    end
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
...
close(videofile);
%% % This loop saves the results for each 30 days in folder Plots_III in png
% or eps format with days in file names

% to check wheather Plots_III folder exists otherwise it makes a folder Plots_III
if not(isfolder('Plots_III'))
    mkdir('Plots_III')
end

for i = 1:10:size(c_total,3)
    figure(9)
    surf(x,y,c_total(:,:,i)')
    view(0,90)
    colorbar
    ...
    saveas(gcf,sprintf('Plots_III/Tumor_III_%ddays.png',3*(i-1)));%png
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_III/Acidity_III_%ddays',3*(i-1)),'epsc');
    
    saveas(gcf,sprintf('Plots_III/Acidity_III_%ddays.png',3*(i-1)))
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_III/Normal_III_%ddays',3*(i-1)),'epsc');
    
    saveas(gcf,sprintf('Plots_III/Normal_III_%ddays.png',3*(i-1)))
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_III/Necrotic_III_%ddays',3*(i-1)),'epsc');
    
    saveas(gcf,sprintf('Plots_III/Necrotic_III_%ddays.png',3*(i-1)))
end
%% To save the workspace
if not(isfolder('Mat_files'))
    mkdir('Mat_files')
end
save('Mat_files/main_2D_III.mat');
%%
toc