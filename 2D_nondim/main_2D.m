...
% in the folder Plots_I and Videos_I
% source term for tumor  = -c1*c*n*(h -1)
% source term for normal cells as given by -(c2*m(h-1)_+ )/(1 + m+ h)  - c3*m
% in this case there are three occlusion sites
% The corresponding reults are included in Figure 4.3 of the thesis
clear all;
clc;
close all;
tic
...
tic;
%% Videos
% In this section Videos are saved in Videos_I folder
% To check if Videos_I folder exists otherwise to create
if not(isfolder('Videos_I'))
    mkdir('Videos_I')
end
...
videofile = VideoWriter(strcat('Videos_I/Tumor_I', video_ext),v_pro);
...
videofile = VideoWriter(strcat('Videos_I/Acidity_I', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
...
videofile = VideoWriter(strcat('Videos_I/Normal_I', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
...
videofile = VideoWriter(strcat('Videos_I/Necrotic_I', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
...
close(videofile);
%% % This loop saves the results for each 30 days in folder Plots_I in png
% or eps format with days in file names

% to check wheather Plots_I folder exists otherwise it makes a folder Plots_I
if not(isfolder('Plots_I'))
    mkdir('Plots_I')
end
...
for i = 1:10:size(c_total,3)
    ...
    saveas(gcf,sprintf('Plots_I/Tumor_I_%ddays.png',3*(i-1)));%png
    ...
    %     saveas(gcf,sprintf('Plots_I/Acidity_I_%ddays',3*(i-1)),'epsc');
    saveas(gcf,sprintf('Plots_I/Acidity_I_%ddays.png',3*(i-1)))
    ...
    %     saveas(gcf,sprintf('Plots_I/Normal_I_%ddays',3*(i-1)),'epsc');
    saveas(gcf,sprintf('Plots_I/Normal_I_%ddays.png',3*(i-1)))
    ...
    %     saveas(gcf,sprintf('Plots_I/Necrotic_I_%ddays',3*(i-1)),'epsc');
    saveas(gcf,sprintf('Plots_I/Necrotic_I_%ddays.png',3*(i-1)))
end
